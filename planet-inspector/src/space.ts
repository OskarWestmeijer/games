...
/**
 * The surface maps: one set, the largest there is. The 8192x4096 day map and the 4096x2048
 * night map, 2.9 MB for the three files.
 *
 * **There used to be a 1.2 MB `4k` set as well**, chosen by a select in the corner of the
 * page and forced on coarse-pointer and save-data devices. It is gone, deliberately: this
 * project exists to look closely at the planet, and a study tool that quietly hands you the
 * blurrier planet on the device you are holding is answering the wrong question. The knob
 * went with it — see `index.html`.
 *
 * It is not free, and the download is the smaller half of the price: an 8192x4096 map with
 * mipmaps is ~180 MB of texture on the GPU, now paid on every device that opens this.
 * `earth-defender/` and `space-station/` carry their own copies of this file and still have
 * both sets — this is the one that dropped one.
 *
 * Clouds are 2048 because NASA publishes that composite at one size only, and it is a soft
 * mask over everything else rather than something you read detail from.
 */
const PLANET_MAPS = { day: day8kUrl, night: night4kUrl, clouds: cloud2kUrl };
...
/**
 * The loaded maps, shared by every `Space` in the page and never disposed. One set, so this
 * is a single promise rather than the cache-keyed-by-quality this used to be: a second scene
 * gets the first one's textures rather than its own 180 MB of them.
 *
...
function loadMaps(maxAnisotropy: number): Promise<PlanetMaps> {
  ...
  const pending = Promise.all([
    loader.loadAsync(PLANET_MAPS.day),
    loader.loadAsync(PLANET_MAPS.night),
    loader.loadAsync(PLANET_MAPS.clouds)
  ])
...
export function createEarthMaterial(
  ...
  function applyMaps(): Promise<void> {
    return loadMaps(maxAnisotropy)
      ...
      .catch((error) => {
        // Not fatal: the shader keeps rendering the procedural fallback rather than going
        // black — see `uHasMaps`.
